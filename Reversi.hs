...
data Player = Player1 | Player2 | EmptyCell deriving (Show, Eq)
 
horizontal :: Integral a => a -> a -> Bool
horizontal old new = div old 8 == div new 8 && boundary new

up :: Integral a => a -> a -> Bool
up old new = succ (div old 8) == div new 8 && boundary new

down :: Integral a => a -> a -> Bool
down old new = pred (div old 8) == div new 8 && boundary new

boundary :: Integral a => a -> Bool
boundary new = (new >= 0) && (new < 64)

updateArray :: (Ix i, MArray a e m) => a i e -> [i] -> e -> m ()
...
goDirection current old arr diff func xs currentPlayer playerSuccess =
  if (func old current)
    then do
...
gameFinished :: (Num a, Ord a) => a -> IO ()
gameFinished player1Count =
  if player1Count == 32
